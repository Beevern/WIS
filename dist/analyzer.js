...
function parseDscan(paste) {
    ...
    for (const rawLine of paste.split("\n")) {
        ...
        if (!shipType || shipType === "" || shipType === "-")
            continue;
        if (EXCLUDED_TYPES.test(shipType))
            continue;
...
async function analyze(req) {
    ...
    const nameToIdLower = new Map([...nameToId.entries()].map(([k, v]) => [k.toLowerCase(), v]));
    if (nameToId.size === 0) {
    ...
    if (dscanShips.length > 0) {
        ...
        try {
            const resp = await (await Promise.resolve().then(() => __importStar(require("axios")))).default.post("https://esi.evetech.net/latest/universe/ids/", shipNames, {
        ...
        if (unresolved.length > 0) {
            ...
            await Promise.all(unresolved.map(async (name) => {
                const id = await (0, eveApi_1.resolveTypeName)(name);
                shipTypeIds.set(name, id);
            }));
        }
        // Mark any that still failed as null
        for (const { shipName } of dscanShips) {
            if (!shipTypeIds.has(shipName))
                shipTypeIds.set(shipName, null);
        }
    }
    ...
    await Promise.all(pilotsToAnalyze.map(async (pilotName) => {
        ...
        const char = await (0, eveApi_1.getCharacter)(characterId);
        ...
        try {
            const killEntries = await (0, eveApi_1.getCharacterShipKills)(characterId, cutoff);
        ...
        await Promise.all(resolvedShips.map(async ([shipName, typeId]) => {
            // Fetch losses (existing behaviour)
            const zkEntries = await (0, eveApi_1.getCharacterShipLosses)(characterId, typeId, cutoff);
            console.log(`  [zkill] ${pilotName} × ${shipName}: ${zkEntries.length} losses`);
            await Promise.all(zkEntries.map(async (zkEntry) => {
                ...
                if (!killmailTime)
                    return;
                if (new Date(killmailTime).getTime() < cutoff)
                    return;
                const fitting = km ? await (0, fitting_1.parseFitting)(km.victim.items ?? []) : [];
                matchedLosses.push({
    ...
    const ships = await Promise.all(dscanShips.map(async ({ shipName, count, onGrid }) => {
        ...
        const stats = typeId ? await (0, eveApi_1.getShipStats)(typeId) : null;
